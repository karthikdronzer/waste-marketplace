...
// Industry views only their own listings, regardless of status
router.get('/my-listings', authMiddleware, async (req, res) => {
  try {
    if (req.user.role !== 'industry') {
      return res.status(403).json({ message: 'Only industry accounts have listings to manage' });
    }
    const listings = await WasteListing.find({ postedBy: req.user.id }).sort({ createdAt: -1 });
    res.json(listings);
  } catch (error) {
    res.status(500).json({ message: 'Server error', error: error.message });
  }
